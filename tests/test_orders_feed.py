...
def test_total_counter_increases(auth_user,main_page, feed_page):
    ...
    with allure.step("Ждем увеличения счетчика"):
        feed_page.open()
...
@pytest.mark.parametrize("driver", ["chrome", "firefox"], indirect=True)
def test_today_counter_increases(auth_user, main_page, feed_page):
...
@pytest.mark.parametrize("driver", ["chrome", "firefox"], indirect=True)
def test_order_appears_in_work_section(auth_user, main_page, feed_page):
    ...
    with allure.step("Открываем ленту и проверяем В работе"):
        feed_page.open()
        ...
        assert order_number.lstrip('0') in in_progress_list, \
            f"Заказ {order_number} не найден в ленте. Список: {in_progress_list}"
